...
function InputHandler( uiMsg, wParam, lParam )
    ...
    then
        if( wParam == Keys.VK_ESCAPE or wParam == Keys.VK_RETURN ) then
			if (g_DiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_DISCUSS_COOP_WAR) then
				--Sorry, no interest in war
				OnButton1();
			elseif (g_DiploUIState == DiploUIStateTypes.DIPLO_UI_STATE_BLANK_DISCUSSION_MEAN_HUMAN) then
				--We're sorry this has caused a divide between us
				OnButton1();
			elseif(Controls.DenounceConfirm:IsHidden() and Controls.WarConfirm:IsHidden()) then
