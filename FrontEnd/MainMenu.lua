...
function OnStartAnimating()
	if Controls.FirstShowDelay:IsStopped() then
		Controls.FirstShowDelay:RegisterAnimCallback( function() end );
		StartAnimating();
	end
end
...
function ShowWindow()
	...
	if ( m_isFirstShow ) then		
		...
		Controls.FirstShowDelay:RegisterAnimCallback( OnStartAnimating );
...
function OnTestClick()
    UIManager:QueuePopup( Controls.TestCivBEUIScreen, PopupPriority.TestCivAEUIScreen );
end
Controls.TestCivBEUIButton:RegisterCallback( Mouse.eLClick, OnTestClick );
